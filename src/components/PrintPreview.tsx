...
function NutritionRow({ label, value, bold }: { label: string; value: string; bold?: boolean }) {
  return (
    <div className={`flex justify-between border-b border-gray-100 px-4 py-2 text-sm ${bold ? "font-black" : ""}`}>
      <span className={bold ? "text-lg" : "text-gray-700"}>{bold ? `${value} kcal` : label}</span>
      <span className={bold ? "text-lg text-terra-600" : "font-semibold text-gray-900"}>{bold ? "Calories" : value}</span>
    </div>
  );
}

function NutritionBlock({ nutrition }: { nutrition: Nutrition }) {
  const rows = [
  ...
  return (
    <div className="mt-10">
      <h2 className="mb-3 text-lg font-extrabold tracking-tight text-gray-900">Nutrition Facts</h2>
      <p className="mb-3 text-xs text-gray-400">Per serving · Estimates only</p>
      <div className="overflow-hidden rounded-xl border border-gray-200">
        <div className="border-b-4 border-gray-900 bg-gray-900 px-4 py-3">
          <p className="text-xl font-black text-white">Nutrition Facts</p>
          <p className="text-xs text-gray-400">Per serving</p>
        </div>
        <div className="bg-white">
          {rows.map((row) => (
            <NutritionRow key={row.label} {...row} />
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-4 px-6 py-3">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <label className="flex cursor-pointer select-none items-center gap-2 text-sm font-medium text-gray-600 hover:text-gray-900">
              <div className={`relative flex h-5 w-5 items-center justify-center rounded border-2 transition-colors ${showImage ? "border-terra-600 bg-terra-600" : "border-gray-300 bg-white"}`}>
                {showImage && (
            ...
            {recipe.nutrition && (
              <label className="flex cursor-pointer select-none items-center gap-2 text-sm font-medium text-gray-600 hover:text-gray-900">
                <div className={`relative flex h-5 w-5 items-center justify-center rounded border-2 transition-colors ${showNutrition ? "border-terra-600 bg-terra-600" : "border-gray-300 bg-white"}`}>
                  {showNutrition && (
                    <svg viewBox="0 0 12 10" className="h-3 w-3 fill-none stroke-white stroke-2">
                      <polyline points="1,5 4,8 11,1" />
                    </svg>
      ...
      <div className="mx-auto max-w-3xl px-8 py-10">
        ...
        <p className="mt-3 text-base leading-relaxed text-gray-500">{recipe.description}</p>
        ...
        <div className="mt-6 flex flex-wrap gap-x-8 gap-y-4 border-y border-gray-200 py-5">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Prep Time</div>
              <div className="text-sm font-bold text-gray-900">{recipe.prep_time}</div>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            <Flame className="size-5 shrink-0 text-orange-500" />
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Cook Time</div>
              <div className="text-sm font-bold text-gray-900">{recipe.cook_time}</div>
            </div>
          </div>
          <div className="flex items-center gap-2.5">
            <Users className="size-5 shrink-0 text-terra-500" />
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Servings</div>
          ...
          {recipe.nutrition?.calories && (
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <div className="text-[10px] font-semibold uppercase tracking-widest text-gray-400">Calories</div>
                <div className="text-sm font-bold text-gray-900">{recipe.nutrition.calories} kcal</div>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="mt-12 border-t border-gray-100 pt-6 text-center text-xs text-gray-400">
          Recipe from <span className="font-semibold text-terra-600">LowKeyCooking</span>
        </div>
      </div>
    </div>
  );
